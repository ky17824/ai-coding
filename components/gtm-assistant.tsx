...
export function GtmAssistant({ assessment, actions, initialPlan }: Props) {
  ...
  const [question, setQuestion] = useState<GtmAssistantQuestion | null>(
    pendingQuestion
      ? {
          ...
          reason: "이 답변을 반영해 실행계획을 이어서 작성합니다.",
          inputType: "text",
          options: [],
      ...
      if (payload.result.kind === "next_question") {
        ...
        setNotice(
          payload.result.generatedBy === "deterministic-fallback"
            ? "AI 연결 없이 진단 액션으로 안전한 기본 계획을 만들었습니다."
            : "AI GTM 실행계획 초안을 만들었습니다."
        );
      }
      setMessage("");
    } catch (error) {
  ...
  async function approve() {
    ...
    if (response.ok) {
      ...
      setNotice("계획을 승인했습니다. GTM 여정에 연결했습니다.");
    } else {
  ...
  return (
    <div className="app-container assistant-layout">
      <aside className="assistant-sidebar panel">
        ...
        <h1>진단을 실행계획으로</h1>
        <p>55문항 결과와 저장된 액션만 사용해 30·60·90일 계획을 함께 만듭니다.</p>
      ...
      <section className="assistant-workspace">
        <div className="question-heading">
          ...
          <h2>대표님의 조건을 알려주세요.</h2>
          <p>고객명·연락처·계약서 원문은 입력하지 마세요. 목표와 제약만 있으면 됩니다.</p>
        </div>
        <div className="assistant-context panel">
          ...
          <label>가용 자원<input value={context.resources} onChange={(event) => setContext({ ...context, resources: event.target.value })} placeholder="예: 대표 1명, 월 300만원" /></label>
          <label>목표 기한<input type="date" value={context.deadline} onChange={(event) => setContext({ ...context, deadline: event.target.value })} /></label>
          <label className="assistant-context__wide">제약<textarea rows={2} value={context.constraints} onChange={(event) => setContext({ ...context, constraints: event.target.value })} placeholder="예: 현지 법인 설립 전 고객 검증이 필요함" /></label>
        </div>
        ...
        <div className="assistant-prompt panel">
          ...
          <label>
            ...
            <textarea rows={3} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="선택 사항입니다. 비워두면 현재 정보로 계획을 만듭니다." />
          </label>
              ...
              {items.map((item, index) => (
                <article className="assistant-plan-item panel" key={item.id ?? `${item.title}-${index}`}>
                  ...
                  <div className="assistant-plan-fields">
                    ...
                    <label className="assistant-context__wide">완료 증거<input value={item.completionEvidence} onChange={(event) => updateItem(index, { completionEvidence: event.target.value })} /></label>
                  </div>
