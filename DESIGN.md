...
- Signature — Stage Gate 막대 (2026-08-19, C단계): `components/stage-gate-bar.tsx` 하나가 준비 1·2·3단계를 세 칸(6px 간격)으로 나눈 진행 막대와 칸마다의 통과 기준선(`--gate`, 80%)을 그린다. 현재 단계 칸은 mint 트랙, 통과한 칸은 green-dark 채움. `md`(라벨·값 포함, 10px)는 대시보드·랜딩 히어로 미리보기, `sm`(6px, 막대만)은 여정 상단에 쓴다. 대시보드 첫 진입에서만 600ms로 한 번 채워지고 reduced-motion에서는 즉시. 다른 색·아이콘을 더하지 않는 것이 규칙이며, 새 화면에서 “현재 단계”를 보여 줘야 하면 이 컴포넌트를 쓴다.
- Korean copy rule scope (2026-08-19): 한글 표기 통일(유사 사례·전문가 검증·시장 규모 등)은 화면 라벨뿐 아니라 **모델 지시문(프롬프트)까지** 적용한다 — 모델은 지시문 어휘를 보고서 본문에 그대로 쓰기 때문이다(`lib/ko-copy-lint.test.ts`가 `lib/ai-agent-report.ts`·`lib/catalog/rules.ts`도 검사). 이미 저장된 보고서 본문은 재생성하지 않고 표시 계층(`ai-agent-workspace`의 `DISPLAY_TERMS`)에서 옛→새 표기 한 단어 쌍만 보정한다. 이 사전은 범용 교정기로 키우지 않는다.
