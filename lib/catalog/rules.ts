...
export const COMPLETION_INSTRUCTIONS: Record<string, Copy> = {
  "ai-market-intelligence": { ko: "TAM·SAM·SOM·교두보 시장은 Top-Down과 Bottom-Up을 각각 산정한 뒤 low/base/high로 교차검증하고 산식·연도·통화를 제시하세요. 교두보 시장은 SAM 안의 최초 공략 세그먼트 전체 규모(100% 점유 가정), SOM은 3~5년 안에 실제 획득 가능한 매출이므로 저·기준·고 모두 TAM ≥ SAM ≥ 교두보 ≥ SOM을 지키세요. 직접·인접·대체 경쟁구도를 포함하세요.", en: "Estimate TAM, SAM, SOM, and the beachhead market separately with Top-Down and Bottom-Up methods, triangulate low/base/high, state formula, year, and currency. The beachhead is the full size of the first segment inside SAM (100% share) and SOM is the revenue obtainable within 3–5 years, so keep TAM ≥ SAM ≥ beachhead ≥ SOM at low, base, and high. Include direct, adjacent, and substitute competition." },
  ...
  "ai-entry-requirements": { ko: "규제 요건은 공식출처를 우선하고 제품분류·법률·세무·인허가·계약 효력은 전문가 검증 필요로 남기세요.", en: "Prioritize official regulatory sources and leave classification, legal, tax, approval, and contract-effect conclusions for human verification." },
  "ai-partner-research": { ko: "후보별 역할·선정근거·최근 활동·검증 질문·대체경로를 제시하고 공개정보와 실제 관계·의향을 구분하세요.", en: "For each candidate give role, rationale, recent activity, validation questions, and alternatives; distinguish public evidence from actual relationships or intent." },
 ...
 */
export const OFFICIAL_SOURCE_AGENT_ID = "ai-entry-requirements";
...
export function buildSpecialistRules(version: SurveyVersion) {
  ...
  for (const question of getIntakeQuestions("ko", version)) {
    const owner = question.itemId === "target-market" ? "ai-market-intelligence"
      ...
      : ["lpa-infra-partner", "lpa-bridge-person"].includes(question.id) || ["partner-acquisition", "partner-contract"].includes(question.itemId) ? "ai-partner-research"
