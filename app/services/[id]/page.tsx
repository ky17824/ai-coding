      ...
      <div className="app-container service-detail">
        <section className="service-main panel">
          ...
          <div className="detail-block"><h2>{en ? "How it works" : "진행 방식"}</h2>{isAi ? <ol>
            ...
            <li>{en ? "The AI asks up to two material follow-up rounds. Unknown answers become labelled analog assumptions, not facts." : "AI는 결과에 영향을 주는 질문만 최대 2차례 드립니다. ‘모름’으로 답한 항목은 사실로 간주하지 않고 유사 사례를 바탕으로 한 가정으로 표시합니다."}</li>
            <li>{en ? "After you review assumptions, the frontier model produces a report and action plan with traceable sources." : "가정을 확인하면 AI 모델이 출처를 명시한 보고서와 실행 계획을 작성합니다."}</li>
          ...
          {isAi && <><div className="detail-block"><h2>{en ? "Required inputs" : "필요 정보"}</h2><ul>{service.requiredInputs?.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="detail-block"><h2>{en ? "Limits of this service" : "이 서비스의 한계"}</h2><ul>{service.humanVerification?.map((item) => <li key={item}>{item}</li>)}</ul></div></>}
