    ...
    <main className="app-page">
      ...
      <div className="app-container service-detail">
        <section className="service-main panel">
          ...
          {isAi && <><div className="detail-block"><h2>{en ? "Required inputs" : "필요정보"}</h2><ul>{service.requiredInputs?.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="detail-block detail-block--boundary"><h2>{en ? "Limits of this service" : "이 서비스의 한계"}</h2><ul>{service.humanVerification?.map((item) => <li key={item}>{item}</li>)}</ul></div></>}
