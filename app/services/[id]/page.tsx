    ...
    <main className="app-page">
      ...
      <div className="app-container service-detail">
        <section className="service-main panel">
          ...
          {isAi && <><div className="detail-block"><h2>{en ? "Required inputs" : "필요정보"}</h2><ul>{service.requiredInputs?.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="detail-block"><h2>{en ? "Human verification boundary" : "사람 검증 경계"}</h2><ul>{service.humanVerification?.map((item) => <li key={item}>{item}</li>)}</ul></div></>}
