...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: "론칭 대상·고객 문제·핵심 가치·목표국가·목표 고객군을 입력해 주세요." },
      { status: 400 }
    );
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    return NextResponse.json(
      { message: "AI 시장 조사를 사용할 수 있도록 OpenAI API 설정을 확인해 주세요." },
      { status: 503 }
    );
  }
  ...
  if ((existingPlan?.market_research_count ?? 0) >= 3) {
    return NextResponse.json(
      { message: "시장·경쟁 사전조사 3회 한도에 도달했습니다. 현재 결과를 확인해 주세요." },
      { status: 429 }
    );
  }
  ...
  try {
    ...
    if (!response.output_parsed?.result) {
      throw new Error("구조화된 시장 조사 결과가 없습니다.");
    }
    const result = finalizeMarketResearch(response.output_parsed.result);
    ...
    if (!planId) {
      ...
      if (error || !created) throw error ?? new Error("계획을 시작하지 못했습니다.");
      planId = created.id;
    } else {
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "시장·경쟁 사전조사를 만들지 못했습니다." },
      { status: 500 }
    );
  }
}
