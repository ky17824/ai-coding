...
export async function GET(
  ...
  const marketSizes = research.marketSizing.map((entry) => {
    ...
    const sources = entry.sources.length > 0
      ...
      : "";
    const calculationInputs = entry.calculationInputs.length > 0 ? `<h4>${en ? "Calculation inputs" : "계산 입력값"}</h4>${list(entry.calculationInputs.map((input) => `${input.name}: ${input.low}–${input.high} (${en ? "base" : "기준"} ${input.base}) ${input.unit} · ${citations(input.sources)}`))}` : "";
