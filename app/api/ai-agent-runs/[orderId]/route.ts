...
export async function POST(request: Request, { params }: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    ...
    const reportInstructions = `${buildAiAgentInstructions(parsed.data.locale, service.title, service.deliverables)} ${(service.completionInstructions ?? []).join(" ")} ${en ? `questionCoverage must contain exactly the question IDs listed below — each once, no others (do not add not-applicable questions from readinessApplicability). Order Critical, current_gate, low_score, other. If you did not use one of the listed questions, keep it in the list with disposition excluded and a reason. Market sizing: use the target country's currency as an ISO 4217 code (e.g. USD, JPY, KRW), and for each of TAM, SAM, beachhead, and SOM give method, a one-line formula with the actual numbers, and the low/base/high assumptions as separate list items. Definitions: beachhead = the full size of the first segment you attack inside SAM (100% share); SOM = revenue realistically obtainable within 3–5 years, mostly inside that beachhead — so TAM ≥ SAM ≥ beachhead ≥ SOM must hold at low, base, and high. Cite source URLs only from publicEvidence. Resolve or explicitly record every contradiction. Required question IDs with the priority you MUST copy verbatim into questionCoverage.priority — do not re-judge them: ${questionPriorityList}.` : `questionCoverage에는 아래 목록의 문항 ID만, 각각 정확히 한 번 넣으세요 — readinessApplicability의 해당 없음 문항 등 목록 밖 ID는 넣지 마세요. Critical, current_gate, low_score, other 순서로 정렬하세요. 목록의 문항 중 사용하지 않은 것은 빼지 말고 disposition을 excluded로 두고 이유를 적으세요. 시장 규모: 통화는 조사 대상국의 통화를 ISO 4217 코드(USD, JPY, KRW 등)로 적고, TAM·SAM·교두보·SOM 각각에 산정 방식(method), 실제 숫자가 들어간 한 줄 산식(formula), 저/기준/고 가정을 항목별로 나눈 assumptions를 적으세요. 정의: 교두보 = SAM 안에서 처음 공략할 세그먼트의 전체 규모(100% 점유 가정), SOM = 3~5년 안에 실제로 획득 가능한 매출(대체로 그 교두보 안에서 실현). 따라서 저·기준·고 모두 TAM ≥ SAM ≥ 교두보 ≥ SOM이어야 합니다. 출처 URL은 publicEvidence에 있는 것만 사용하세요. 모순은 모두 해결하거나 명시적으로 기록하세요. 각 문항의 priority는 아래 값을 **그대로** 복사하세요. 직접 판단하지 마세요: ${questionPriorityList}.`}`;
