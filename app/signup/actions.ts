...
export async function signUpWithPassword(
  ...
  if (!parsed.success) {
    ...
    return {
      ok: false,
      message: en ? "Review the highlighted account details." : "입력하신 가입 정보를 확인해 주세요.",
  ...
  try {
    ...
    return {
      ok: false,
      message: en ? "Review the highlighted account details." : "입력하신 가입 정보를 확인해 주세요.",
      fieldErrors: { phone: en ? "Enter a valid phone number." : error instanceof Error ? error.message : "번호를 확인해 주세요." }
    };
  }
  ...
  if (error) return { ok: false, message: en ? "We could not send the verification email. Please try again in a few minutes." : "인증 메일을 보내지 못했습니다. 잠시 후 다시 시도해 주세요." };
  return {
    ok: true,
    message: en ? "We sent a verification email. Check your inbox to continue." : "입력하신 주소로 메일을 보냈습니다. 메일함을 확인해 주세요."
  };
}
