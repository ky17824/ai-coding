  ...
  return (
    <main className="app-page">
      ...
      <div className="app-container admin-company">
        ...
        <span className="page-kicker">COMPANY OPERATIONS</span>
        ...
        <section className="admin-section"><h2>액션</h2><div className="table-scroll panel"><table className="admin-table"><thead><tr><th>우선순위</th><th>액션</th><th>담당</th><th>기한</th><th>상태</th></tr></thead><tbody>{(actions ?? []).map((action) => <tr key={action.id}><td>{action.urgency}</td><td>{action.title}</td><td>{action.owner_label}</td><td>{action.due_date ?? "-"}</td><td>{action.completed_at ? "완료" : "진행 전"}</td></tr>)}</tbody></table></div></section>
