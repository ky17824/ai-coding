...
작성일: 2026-08-10
범위: AI GTM 어시스턴트의 론칭 제품·서비스·솔루션 정의, 시장동향, 시장규모, 경쟁사 사전조사, 준비 3단계 후 실제 판매 가능성 예비검증, 창업자 확인, 단계별 실행계획 연결
...
2. `준비 1단계`와 `준비 2단계`에서는 후속 고객 행동·지불·반복 구매 증거를 모두 모을 수 없으므로 판매 가능성을 판정하지 않는다.
3. 두 단계에서는 `AI 시장·경쟁 사전조사`, 검증할 가설, 다음에 수집할 증거를 제공하고 다운로드 보고서에 포함한다.
4. `준비 3단계`까지 55문항을 모두 답한 경우에만 55개 답변과 제출 증거를 사용해 `실제 판매 가능성 예비검증`을 제공한다. 미응답·잠김 문항은 실패나 0점으로 계산하지 않는다.
...
| --- | --- | --- |
| 준비 1단계 | 시장·경쟁 사전조사 + 검증 가설 + 다음 증거 | 포함 |
| 준비 2단계 | 시장·경쟁 사전조사 + 검증 가설 + 다음 증거 | 포함 |
| 준비 3단계 문항까지 55개 모두 응답 | 시장·경쟁 사전조사 + 실제 판매 가능성 예비검증 | 포함 |
...
2. **시장·경쟁 사전조사**: AI가 내부 위키와 공개 웹 자료를 사용해 시장동향, 시장규모, 주요 경쟁사와 확인 공백을 제시한다. `준비 1단계·준비 2단계`에서는 여기까지를 보고서로 제공한다. 준비 3단계까지 55문항을 모두 답한 경우에만 같은 단계 안에서 실제 판매 가능성 예비검증을 추가한다.
...
- 준비 1단계·준비 2단계 결과에서는 실제 판매 가능성을 판정하지 않고 사전조사, 검증 가설, 다음 증거를 다운로드 보고서에 포함한다.
- 준비 3단계까지 55문항을 모두 답한 경우에만 실제 판매 가능성 예비검증을 제공한다.
...
flowchart LR
  ...
  C --> D["시장규모·동향·경쟁사 검토"]
  D -->|"준비 1단계·준비 2단계"| H["검증 보류·사전조사 보고서"]
  D -->|"준비 3단계·55문항 완료"| I["실제 판매 가능성 예비검증"]
  D -->|"가정 수정"| C
  H --> E["3 실행 계획"]
  I --> E
  E --> F["계획 승인·여정 반영·보고서"]
...
- `실제 판매 가능성 검증은 준비 3단계 진단 후 제공됩니다` 안내를 조사 결과 아래에 표시한다.
...
- 현재 계획 편집 UI를 유지한다.
- 상단에 확인된 론칭 정의와 시장·경쟁 사전조사 요약을 접을 수 있는 카드로 제공한다. 준비 3단계 55문항 완료 진단에는 판매 가능성 예비검증 요약도 포함한다.
...
- 준비 3단계 전에는 `deferred`만 반환하고 모델에게 판매 가능성 판정을 요청하지 않음
- 준비 3단계 후에도 AI 요약과 실제 답변·증거를 별도 필드로 유지함
...
- `plan`: `market_research.status === "confirmed"`인 경우에만 AI 계획을 생성한다. 준비 1단계·준비 2단계에서는 사전조사와 검증 가설을, 준비 3단계 55문항 완료에서는 확인된 예비검증도 입력에 포함한다. 보조 fallback을 명시적으로 선택한 경우는 예외로 하고 보고서에 조사 미완료를 기록한다.
...
- 준비 1단계·준비 2단계에는 검증 보류 안내와 보고서 포함 항목을, 준비 3단계 55문항 완료에는 차원별 판매 가능성 예비검증을 같은 2단계 안에서 조건부 표시한다.
...
- 준비 1단계·준비 2단계 보고서에는 `실제 판매 가능성 검증 보류`, 검증 가설과 다음 증거를 표시한다. 준비 3단계 55문항 완료 보고서에는 차원별 예비검증, 근거 문항·증거와 제한을 추가한다.
...
16. 준비 1단계·준비 2단계 진단은 미응답·잠김 문항을 부정 근거로 사용하지 않고 판매 가능성을 `검증 보류`로 표시한다.
17. 준비 1단계·준비 2단계 HTML 보고서에는 판매 가능성 판정 대신 검증 가설과 다음에 모을 고객 행동·지불 증거가 포함된다.
18. 현재 55개 기대 question ID가 모두 응답된 준비 3단계 진단만 `실제 판매 가능성 예비검증`을 요청·표시할 수 있다.
...
- 브라우저 확인: 새 계획·과거 계획, 론칭 정의 저장, 조사 로딩·완료·실패·재시도, 준비 1단계·준비 2단계 검증 보류, 준비 3단계 55문항 예비검증, 가정 수정, 창업자 확인, 계획 생성·승인, 새로고침 복원.
...
| 사전조사가 판매 검증으로 오해됨 | 단계명을 `사전조사`로 고정하고 준비 3단계 전에는 검증 보류 문구와 다음 증거만 표시 |
