...
- 최신 진단이 없는 신규 창업자: 기존처럼 준비 1단계 질문을 시작한다.
...
1. 사용자가 로그인하고 준비 1단계 또는 준비 2단계 판정을 받은 뒤 다시 로그인하면 진단 첫 문항이 아니라 대시보드를 먼저 본다.
...
- `AssessmentForm`은 기본 상태를 빈 답변, 준비 1단계, 잠금 단계 0으로 초기화한다: `components/assessment-form.tsx:44-56`.
...
3. 최신 진단이 없는 로그인 사용자는 `/assessment`에서 준비 1단계 질문을 시작한다.
...
5. 프로덕션과 동일한 환경에서 다음 두 계정으로 브라우저 스모크 테스트를 수행한다.
   - 진단 이력이 없는 신규 계정: 로그인 → 준비 1단계 진단.
   - 준비 1단계 또는 준비 2단계 이력이 있는 재방문 계정: 로그인 → 대시보드 → 지난 응답 → AI 계획 상태.
