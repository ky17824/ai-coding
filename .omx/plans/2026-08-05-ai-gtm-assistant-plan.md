...
- AI API와 실제 진단 단계 타입은 `준비 1단계/준비 2단계/준비 3단계/진출 실행 가능`으로 통일한다 (`app/api/ai/explain/route.ts:7-20`, `lib/types.ts:3-5`).
...
- **준비 1단계:** 진출 목적 합의, 중단 기준, 자원 우선순위, 책임자 지정, 본국 PMF 자료 정리. AI는 합의 문서와 체크리스트를 돕는다.
- **준비 2단계:** 후보시장 비교, 고객 인터뷰 설계, 총 진입비용, 규제 체크리스트, 첫 수요 신호 검증. 실제 인터뷰·규제 판단은 전문가에게 전환한다.
- **준비 3단계:** GTM 실험, KPI, 파트너 역할·계약, 현지 운영, 의사결정권과 스케일 조건. 실제 파트너 발굴·영업·협상·현지화 실행은 전문가에게 전환한다.
...
- `app/api/ai/explain/route.ts:7-20`의 단계 enum을 `lib/types.ts:5`와 계속 일치신킨다.
...
4. 내부 계정으로 준비 1단계·준비 2단계·준비 3단계 fixture를 각각 1회 검증한다.
...
19. 준비 1단계·준비 2단계·준비 3단계와 국가·산업을 섞은 검색 fixture 12개 중 10개 이상에서 기대 위키 문서가 상위 8개 안에 포함된다.
...
- 준비 1단계: 목적 합의와 자원 계획을 생성하고 AI 지원 항목을 완료 처리
- 준비 2단계: 시장 인터뷰 계획을 만들고 현장 인터뷰를 전문가로 전환
- 준비 3단계: GTM 실험과 파트너 계획을 만들고 협상 업무를 전문가로 전환
