...
- 초기 목표국가(Target Country)와 목표 고객군(Target Customer Segment)은 준비 2단계까지 창업자가 직접 확정해야 하며, 둘 중 하나라도 없으면 준비 3단계 문항을 열지 않는다.
...
- 준비 3단계의 부분 통과: 초기 운영값으로 60% 이상 80% 미만이고 Critical blocker가 0건인 경우를 제안한다.
- Critical blocker가 있으면 점수와 관계없이 부분 통과가 아니라 미통과다.
- Gate A는 기존 점수·Critical 규칙만 사용한다. 목표국가와 목표 고객군을 아직 확정하지 않았다는 이유만으로 준비 1단계를 막지 않는다.
- Gate B는 기존 점수·Critical 규칙에 더해 `초기 목표국가`와 `목표 고객군`이 모두 직접 입력·확정되어야 통과한다. 하나라도 없으면 준비 3단계는 잠긴다.
...
| --- | --- | --- |
| 준비 1단계 미통과 | 준비 1단계 판정·격차 화면 → AI 워크숍 | 30일만 |
| 준비 1단계 통과 | 준비 2단계 문항 해제 | 없음 |
| 준비 2단계 미통과 + 준비 1단계 90% 이상·P0 없음 | 준비 2단계 판정·격차 화면 → AI 워크숍 | 60일만 |
| 준비 2단계 미통과 + 준비 1단계 80~89% 또는 미완료 P0 있음 | 준비 2단계 판정·격차 화면 → AI 워크숍 | 30일 + 60일 |
| 준비 2단계 통과 | 준비 3단계 문항 해제 | 없음 |
| 준비 3단계 미통과 또는 Critical blocker 있음 | 준비 3단계 판정·격차 화면 → AI 워크숍 | 30일만 |
| 준비 3단계 부분 통과 | 준비 3단계 판정·격차 화면 → AI 워크숍 | 30일 + 60일 |
| 준비 3단계 통과 | 진출 실행 계획 워크숍 | 30일 + 60일 + 90일 |
...
```mermaid
flowchart LR
  A["준비 1단계 문항"] --> B{"Gate A 판정"}
  B -->|"미통과"| C["준비 1단계 격차 + 30일 AI 계획"]
  B -->|"통과"| D["준비 2단계 문항"]
  D --> T["초기 목표국가·목표 고객군 확인"]
  T --> E{"Gate B 판정"}
  E -->|"미통과"| F["준비 2단계 격차 + 60일 또는 30·60일 AI 계획"]
  E -->|"통과"| G["준비 3단계 문항"]
  G --> H{"Gate C 판정"}
...
- 좌측에는 전체 `0/55` 대신 `준비 1단계 0/N`처럼 현재 단계 진행률을 먼저 보여준다.
...
- 준비 2단계 화면에는 55문항과 별도로 `준비 3단계 전 초기 목표시장 확인` 카드가 있다. 목표국가와 목표 고객군을 직접 입력하고 확인하며, 준비 1단계 또는 AI 워크숍에서 이미 확정한 값이 있으면 그대로 불러온다.
...
- 통과 시 주 CTA는 `준비 2단계 진단 계속`, `준비 3단계 진단 계속`이다.
- Gate B 점수와 Critical 조건을 충족했지만 목표시장 두 값 중 하나가 없으면 결과는 `질문 기준은 충족했습니다. 초기 목표시장을 확정하면 준비 3단계가 열립니다.`로 표시하고 주 CTA를 `초기 목표시장 확정`으로 바꾼다.
...
- 제목은 현재 Gate에 따라 `준비 2단계으로 넘어가기 전 확인할 항목` 또는 `준비 3단계로 넘어가기 전 확인할 항목`으로 한 번만 표시한다.
...
- `POST /api/assessments` 입력을 `{ assessmentId?, stageId, answers, offering, targetMarket? }`으로 바꾸고, 해당 단계의 정확한 question id 집합만 허용한다. `targetMarket`은 준비 2단계 제출에서만 저장하며 두 값이 모두 있을 때 확인 시각을 서버가 기록한다.
...
3. `components/assessment-form.tsx`, `app/globals.css`, `lib/pending-assessment.ts`
   - 현재 단계만 표시하고 준비 2단계 목표시장 확인, 잠금·판정·다음 단계/AI 분기를 구현한다.
...
1. 새 사용자는 준비 1단계 문항만 볼 수 있고 준비 2단계·준비 3단계 문항을 열 수 없다.
2. 준비 1단계 미통과 시 준비 2단계 문항 없이 30일 AI 계획 CTA가 나온다.
3. 준비 1단계 통과 시 같은 진단 세션에서 준비 2단계 문항이 열린다.
4. 준비 2단계 미통과 시 준비 1단계 완성도 규칙에 따라 `60` 또는 `30·60` 기간만 AI 계획에 나타난다.
5. 준비 3단계 결과는 통과·부분 통과·미통과를 구분하고 규칙에 맞는 기간만 생성한다.
...
16. 준비 1단계는 목표국가·목표 고객군이 미확정이어도 기존 Gate A 규칙으로 판정된다.
17. 준비 2단계은 점수와 Critical 조건을 충족해도 목표국가 또는 목표 고객군이 미확정이면 준비 3단계가 열리지 않는다.
...
- 브라우저 확인: 세 단계 잠금/해제, Gate A 목표시장 미확정 허용, Gate B 목표시장 미확정 차단과 확정 후 준비 3단계 해제, 반복 문구 없는 선결 조건 카드, 질문 상태 필터, 질문-계획 연결, 근거 첨부, Gate 재확인 통과·미통과, 로그인 전 응답 복원, AI 질문/계획/fallback, 모바일 1열, 키보드 포커스.
